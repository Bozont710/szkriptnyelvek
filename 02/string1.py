...
def mix_up(a, b):
    #'a' első két karakterét elmentjük
    swap = a[:2]
    #az 'a' első két karakterét 'b' első két karakterére cseréjük
    a = a.replace(a[:2], b[:2])
    #felhasználva az 'a' elmentett első két karakterét lecseréjük 'b' első két karakterére
    b = b.replace(b[:2], swap)
    return f"{a} {b}"
